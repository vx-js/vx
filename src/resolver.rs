use crate::lockfile::{LockNode, Lockfile};
...
impl Resolver {
    ...
    pub async fn resolve(
        ...
        lock.root.requires.retain(|k, _| lock.root.dependencies.contains_key(k));

        let mut queue: VecDeque<(String, BTreeMap<String, String>)> = VecDeque::new();
        queue.push_back(("__root__".to_string(), root_deps));
        ...
        while !queue.is_empty() {
            ...
            while batch.len() < batch_size {
                let Some((parent_key, deps)) = queue.pop_front() else { break };
                if parent_key != "__root__" && !visited_nodes.insert(parent_key.clone()) {
                    continue;
                }
                batch.push((parent_key, deps));
            }
            if batch.is_empty() {
                continue;
            }
            ...
            for (parent_key, deps) in batch {
                if parent_key != "__root__" {
                    if let Some(parent) = lock.packages.get_mut(&parent_key) {
                        parent.requires.retain(|k, _| deps.contains_key(k));
                    }
                }
                for (dep_name, dep_req) in deps {
                    ...
                    if !node.dependencies.is_empty() {
                        queue.push_back((child_key.clone(), node.dependencies.clone()));
                    }
                }
            }
        }
